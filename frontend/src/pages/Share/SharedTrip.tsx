...
export default function SharedTrip() {
  ...
  if (notFound) {
    return (
      <div className="min-h-[60vh] grid place-items-center text-center px-6">
        <div>
          <span className="grid place-items-center w-16 h-16 rounded-2xl bg-red-50 dark:bg-red-500/10 text-red-500 mx-auto mb-4">
            <ShieldAlert size={28} />
          </span>
          <h1 className="text-2xl font-bold text-stone-900 dark:text-stone-50 mb-2">Lien introuvable</h1>
          <p className="text-stone-500 dark:text-stone-400 max-w-sm">Ce lien de suivi n'existe pas ou n'est plus valide.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-xl mx-auto px-4 sm:px-6 py-8">
      ...
      <Card className="mb-4 !p-3" padded={false}>
        <div className="rounded-xl overflow-hidden" style={{ height: '40vh' }}>
          <MapContainer center={center} zoom={13} style={{ height: '100%', width: '100%' }} scrollWheelZoom={false}>
            ...
            {data.origin_lat != null && data.origin_lng != null && (
              <Marker position={[data.origin_lat, data.origin_lng]} icon={pinIcon('#1f9d65', 'A')} />
            )}
            {data.dest_lat != null && data.dest_lng != null && (
              <Marker position={[data.dest_lat, data.dest_lng]} icon={pinIcon('#f2590e', 'B')} />
            )}
      ...
      <Card className="mb-4">
        <div className="flex items-start gap-2 text-sm text-stone-700 dark:text-stone-200">
          <MapPin size={16} className="mt-0.5 text-secondary-600 shrink-0" />
          <div>
            <div>{data.origin}</div>
            <div className="text-stone-400 dark:text-stone-500">→ {data.destination}</div>
          </div>
        </div>
        {data.distance_km != null && (
          <div className="mt-2 text-xs text-stone-400 dark:text-stone-500">{data.distance_km.toFixed(1)} km</div>
        )}
      </Card>
      ...
      {data.driver && (
        <Card>
          <div className="flex items-center gap-3">
            {data.driver.photo ? (
              <img src={data.driver.photo} alt={data.driver.username} className="w-12 h-12 rounded-full object-cover border-2 border-white dark:border-stone-800 shadow-card shrink-0" />
            ) : (
              <span className="grid place-items-center w-12 h-12 rounded-full bg-secondary-500 text-white font-bold uppercase shrink-0">
                {(data.driver.username || '?').slice(0, 2)}
              </span>
            )}
            <div className="min-w-0">
              <div className="font-semibold text-stone-800 dark:text-stone-100 truncate">{data.driver.username || 'Chauffeur'}</div>
              {data.driver.vehicle && (
      ...
      <p className="mt-4 text-center text-xs text-stone-400 dark:text-stone-500">Suivi en direct via samaChauffeur — actualisé automatiquement.</p>
    </div>
  )
}
