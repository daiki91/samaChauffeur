...
import { MapPin, Navigation, Wallet, Route as RouteIcon, X } from 'lucide-react'
...
export default function NewTripOfferModal({ offer, onAccept, onDecline, accepting }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/50 flex items-end sm:items-center justify-center z-[1000] p-0 sm:p-4">
      <div className="bg-white rounded-t-3xl sm:rounded-2xl shadow-floating w-full max-w-sm p-6 relative animate-fade-in-up overflow-hidden">
        <div className="absolute top-0 left-0 h-1 bg-brand-500 transition-all duration-1000 ease-linear" style={{ width: `${progressPct}%` }} />

        <button onClick={onDecline} disabled={accepting} className="absolute right-4 top-5 text-stone-400 hover:text-stone-600" aria-label="Refuser">
          <X size={18} />
        </button>
        ...
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-stone-900">Nouvelle course !</h3>
