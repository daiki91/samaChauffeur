...
const features = [
  ...
  },
  {
    ...
    color: 'bg-accent-300/30 text-accent-600',
  },
  {
    icon: Clock,
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 sm:px-6 py-20 sm:py-28">
      <Reveal className="max-w-2xl mx-auto text-center mb-14">
        <span className="inline-block rounded-full bg-brand-50 text-brand-600 text-xs font-semibold tracking-wide uppercase px-3 py-1 mb-4">Pourquoi samaChauffeur</span>
        <h2 className="text-3xl sm:text-4xl font-extrabold text-stone-900 mb-4">Voyagez l'esprit tranquille, à chaque course</h2>
        <p className="text-stone-500 text-lg">Conçu pour vous simplifier la vie : des chauffeurs de confiance, des prix clairs et une expérience pensée pour que chaque trajet se passe sans accroc.</p>
      </Reveal>

      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {features.map((f, i) => (
          <Reveal key={f.title} delay={i * 90} variant="up">
            <div className="group h-full rounded-3xl border border-stone-100 bg-white p-8 shadow-card hover:shadow-floating hover:-translate-y-1.5 transition-all duration-300">
              <span className={`grid place-items-center w-16 h-16 rounded-2xl mb-6 transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-6 ${f.color}`}>
                <f.icon size={28} />
              </span>
              <h3 className="text-lg font-bold text-stone-900 mb-2">{f.title}</h3>
              <p className="text-[15px] leading-relaxed text-stone-500">{f.text}</p>
            </div>
          </Reveal>
        ))}
      </div>
