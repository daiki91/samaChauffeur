...
const statusStyles: Record<string, string> = {
  ...
  ARRIVED: 'bg-accent-300/40 text-accent-700',
...
export default function Badge({ status, children }: { status?: string; children?: React.ReactNode }) {
  ...
  const key = status || ''
  return <span className={`inline-flex items-center rounded-full text-xs font-semibold px-2.5 py-1 ${statusStyles[key] || 'bg-stone-100 text-stone-600'}`}>{statusLabelsFr[key] || key}</span>
}
