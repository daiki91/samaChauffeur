...
export default function StarRating({ value, onChange, size = 28, readOnly = false, label = 'Note' }: Props) {
  const [hovered, setHovered] = useState<number | null>(null)
  const display = hovered ?? value

  return (
    <div className="flex items-center gap-1" role={readOnly ? undefined : 'radiogroup'} aria-label={readOnly ? undefined : label} onMouseLeave={() => setHovered(null)}>
      {[1, 2, 3, 4, 5].map((n) => {
