...
import { Navigation, Car, Star, Users, Clock3, Route as RouteIcon, Check, MapPin, Phone, MessageCircle, Plus, X, CalendarClock, Share2, ShieldAlert, Tag } from 'lucide-react'
...
function OptionPicker({ options, value, onChange, disabled }: { options: Option[]; value: string; onChange: (v: string) => void; disabled?: boolean }) {
  return (
    <div className="flex flex-wrap gap-1.5">
      {options.map((opt) => (
        <button key={opt.value} type="button" disabled={disabled} onClick={() => onChange(opt.value)} className={`text-xs font-medium px-2.5 py-1.5 rounded-lg border transition-all duration-200 active:scale-95 ${value === opt.value ? 'bg-brand-500 text-white border-brand-500 scale-105 shadow-card' : 'bg-white text-stone-600 border-stone-200 hover:bg-stone-50 hover:scale-105'} disabled:opacity-50 disabled:active:scale-100 disabled:cursor-not-allowed`}>
          {opt.label}
        </button>
      ))}
    </div>
...
export default function RideStatusBar({ activeTrip, completedTrip, completedPaymentStatus, onPayCompleted, onRateTrip, onSkipRating, ratingSubmitting, driverEtaMin, driverRating, originText, destinationText, onOriginChange, onOriginSelect, onDestinationChange, onDestinationSelect, myPosition, originPoint, stops, onAddStop, onRemoveStop, onStopTextChange, onStopSelect, maxStops, scheduleEnabled, onScheduleToggle, scheduledAtInput, onScheduledAtChange, pendingDiscount, routeLoading, estimate, route, submitting, canRequest, onSubmit, vehicleType, onVehicleTypeChange, paymentMethod, onPaymentMethodChange, onCancel, cancelling, onChangeActiveVehicleType, changingVehicleType, onChangeActivePaymentMethod, changingPaymentMethod, onShareTrip, sharingTrip, onSos, sosSubmitting }: Props) {
  ...
  const waitPoint = waiting && activeTrip?.origin_lat != null && activeTrip?.origin_lng != null ? { lat: activeTrip.origin_lat, lng: activeTrip.origin_lng } : null
  ...
  if (!activeTrip && completedTrip) {
    ...
    return (
      <div key="completed" className="animate-fade-in-up">
        ...
        {!isPaid && (
          <>
            {isPending && <div className="rounded-xl bg-amber-50 border border-amber-100 px-4 py-3 mb-3 text-sm text-amber-700 text-center">Paiement envoyé — en attente de validation par le chauffeur…</div>}
  ...
  if (status === 'SCHEDULED') {
    const when = activeTrip!.scheduled_at ? new Date(activeTrip!.scheduled_at).toLocaleString('fr-FR', { weekday: 'long', day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : null
    return (
      ...
      <div className="rounded-xl bg-secondary-50 border border-secondary-100 px-4 py-4 mb-4 animate-pop">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            {driver?.photo ? <img src={driver.photo} alt={driver.username || 'Chauffeur'} className="w-10 h-10 rounded-full object-cover shrink-0 border-2 border-white shadow-card" /> : <span className="grid place-items-center w-10 h-10 rounded-full bg-secondary-500 text-white font-bold uppercase text-sm shrink-0">{(driver?.username || driver?.phone || '?').slice(0, 2)}</span>}
            <div className="font-semibold text-stone-800 truncate">{driver?.username || driver?.phone || 'Chauffeur en route'}</div>
          </div>
          <span className="inline-flex items-center gap-1 text-xs font-semibold text-amber-600 bg-amber-50 rounded-full px-2 py-1 shrink-0">
      ...
      {sosConfirming && (
        <div className="fixed inset-0 bg-stone-900/50 flex items-center justify-center z-[1000] p-4">
          <div className="bg-white rounded-2xl shadow-floating w-full max-w-sm p-6 relative animate-fade-in-up">
            <button onClick={() => setSosConfirming(false)} className="absolute right-4 top-4 text-stone-400 hover:text-stone-600" disabled={sosSubmitting}>
              <X size={18} />
            </button>
            <span className="grid place-items-center w-11 h-11 rounded-xl bg-red-50 text-red-600 mb-3">
              <ShieldAlert size={20} />
            </span>
            <h3 className="text-lg font-semibold text-stone-900">Déclencher l'alerte SOS ?</h3>
